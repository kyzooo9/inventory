...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "UPDATE peralatan SET KODE_PERALATAN = '$kode_peralatan', ID_JENIS = '$nama_jenis', ID_MERK = '$nama_merk', ID_WARNA = '$nama_warna', NAMA_PERALATAN = '$nama_peralatan', TANGGAL_BELI_PERALATAN = '$tanggal_beli_peralatan', STATUS_PERALATAN = '$status_peralatan', JUMLAH_KERUSAKAN_PERALATAN = '$jumlah_kerusakan_peralatan', STATUS_KETERSEDIAAN_PERALATAN = '$status_ketersediaan_peralatan', ATURAN_SERVICE_PERALATAN = '$aturan_service_peralatan', IMAGE_PERALATAN = '$image_peralatan' WHERE ID_PERALATAN = '$id_peralatan'";
    $result = mysqli_query($koneksi, $query); // Simpan hasil query

    if ($result) {
        header("Location: peralatan.php?status=edit_success"); // Status untuk edit berhasil
    } else {
        header("Location: peralatan.php?status=edit_error"); // Status untuk edit gagal
    }
    exit(); 
}
